...
1. **Calibrates** per-output-channel, per-group min/max statistics for each
...
2. **Quantizes** to an unsigned affine grid (`scale`, `zero_point` per
   group), producing 4-bit or 8-bit integer codes.
3. **Packs** INT4 codes two-per-byte into a `uint8` buffer, so the stored
   tensor is genuinely 4 bits/weight rather than 8-bit values that happen to
   fit in `0-15`.
4. **Dequantizes on the fly** at `forward()` time and runs a normal
   `F.linear`, so a `QuantLinear` is a drop-in replacement for `nn.Linear`.
5. **Reports** validation perplexity, real packed-weight byte counts, and
...
|---|---|---|---|---|
| FP32 | 31.078 | 3072.0 KB | 1.00x | 46.5 |
| INT8 | 31.078 | 960.0 KB | 3.20x | 49.5 |
| INT4 | 31.083 | 576.0 KB | 5.33x | 52.7 |

![Size and quality vs precision](results_chart.png)
